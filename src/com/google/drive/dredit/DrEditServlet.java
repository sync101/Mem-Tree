...
public abstract class DrEditServlet extends HttpServlet {
  ...
  public CredentialManager getCredentaialManager(){
  ...
  protected void loginIfRequired(HttpServletRequest req,
    ...
    if (credential == null) {
      ...
      try {
        resp.sendRedirect(credentialManager.getAuthorizationUrl(req.getParameter("state")));
      } catch (IOException e) {
  ...
  protected Credential getCredential(HttpServletRequest req,
      HttpServletResponse resp) {
    /*String userId = new State(req.getParameter("state")).userId;
    if (userId != null) {
  ...
  protected Oauth2 getOauth2Service(Credential credential) {
    return new Oauth2.Builder(TRANSPORT, JSON_FACTORY, credential).setApplicationName("Mem-Tree").build();
  }
