...
import com.orastager.client.DmaUtil;
import com.orastager.client.Node;
import com.orastager.client.StaticVar;
...
/**
	 ...
	 * </p>
	 */
	public class SmartSuggestOracle extends SuggestOracle {
	   ...
	   */
	  private HashMap<String, Node> toRealSuggestions = new HashMap<String, Node>();
	   ...
	   */
	  public SmartSuggestOracle(String whitespaceChars) {
	  ...
	  public void add(Node node) {
	    String candidate = normalizeSuggestion(node.text);
	    // candidates --> real suggestions.
	    toRealSuggestions.put(candidate, node);
	   ...
	   */
	  public final void addAll(Collection<Node> collection) {
	  ...
	  private List<MultiWordSuggestion> convertToFormattedSuggestions(String query,
	    ...
	    for (int i = 0; i < candidates.size(); i++) {
	      ...
	      String formattedSuggestion = toRealSuggestions.get(candidate).text;

	      // Create strong search string.
	      SafeHtmlBuilder accum = new SafeHtmlBuilder();

	      //set child text icon on left side of suggestion
	      if (toRealSuggestions.get(candidate).ti.getChildCount() > 0 && StaticVar.locate.get(toRealSuggestions.get(candidate).ti.getChild(0)).isKeyWord() == false){
		     accum.appendHtmlConstant("<span title=\""+StaticVar.dateFormatter.format(StaticVar.locate.get(toRealSuggestions.get(candidate).ti.getChild(0)).creation_date)+"\">");
	         //accum.appendHtmlConstant("<span class=hashTag>"+DmaUtil.getChildTextType(StaticVar.locate.get(toRealSuggestions.get(candidate).ti.getChild(0)).attr1, StaticVar.locate.get(toRealSuggestions.get(candidate).ti.getChild(0)).attr2)+"</span>");
		     img.setResource(DmaUtil.getChildTextTypeImg(StaticVar.locate.get(toRealSuggestions.get(candidate).ti.getChild(0)).attr1, StaticVar.locate.get(toRealSuggestions.get(candidate).ti.getChild(0)).attr2, false));
	         accum.appendHtmlConstant(img.toString());
	      }else{
			 accum.appendHtmlConstant("<span title=\""+StaticVar.dateFormatter.format(toRealSuggestions.get(candidate).creation_date)+"\">&nbsp;&nbsp;&nbsp;&nbsp;");
	      }
